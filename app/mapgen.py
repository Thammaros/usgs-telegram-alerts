...
def generate_cartopy_map(
    ...
    ax.add_feature(cfeature.LAKES.with_scale("10m"), facecolor="#b0d6f0", edgecolor='black', linewidth=0.4)
    ax.add_feature(cfeature.RIVERS.with_scale("10m"), edgecolor="#4a90e2", linewidth=0.5)
    ...
    # Plot locations
    ax.plot(quake_lon, quake_lat, "ro", markersize=7, label="Epicenter", transform=ccrs.Geodetic())
    ax.plot(bangkok_lon, bangkok_lat, "go", markersize=7, label="Bangkok", transform=ccrs.Geodetic())
    ...
    # Title
    plt.title("Earthquake Epicenter and Distance to Bangkok", fontsize=13, weight="bold", pad=20)
